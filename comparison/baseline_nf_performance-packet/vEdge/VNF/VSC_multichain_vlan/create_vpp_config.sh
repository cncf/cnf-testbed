...
function clean_vpp_config () {
    ...
    VPP_CONF_FILE="vEdge_vsc_vpp.conf"
...
function generate_vpp_config () {
    ...
    for chain in $(seq 0 $(( "${CHAINS}" -1 ))); do
        ...
        append_vpp_config "set int l2 bridge VirtualEthernet0/0/${vEth} $(( ${offset} + 1 ))"
        ((++vEth))
        for bridge in $(seq 2 $(( ("${domains}"/"${CHAINS}") - 1 ))); do
            append_vpp_config "set int l2 bridge VirtualEthernet0/0/${vEth} $(( ${offset} + ${bridge} ))"
            ((++vEth))
            append_vpp_config "set int l2 bridge VirtualEthernet0/0/${vEth} $(( ${offset} + ${bridge} ))"
            ((++vEth))
        done
        append_vpp_config "set int l2 bridge VirtualEthernet0/0/${vEth} $((${offset} + (${NODENESS} + 1 )))"
