...
openstack keypair create oskey -f ~/.ssh/id_rsa.pub
fi

if [ ! "$(openstack server list | grep test${1} | awk '{print $4}')" == "test${1}" ] ; then
network=`openstack network list | awk '/vlan/ {print $4}' | head -1`
openstack  server create test${1} --flavor ${FLAVOR} --image xenial --nic net-id=${network} --config-drive True --user-data /tmp/test${1}.cfg --key-name oskey --poll
fi

if [ "$(openstack network list | awk '/netext/ {print $4}')" == "netext" ] ; then
float=$(openstack floating ip create netext | awk '/floating_ip_address/ {print $3}')
port_ip=$(openstack server list | grep test${1} | awk '{print $8}')
