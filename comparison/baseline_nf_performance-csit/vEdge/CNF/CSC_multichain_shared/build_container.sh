...
function docker_build_image () {
    # Build docker image.
    if [ -z "$(docker image list | grep cnf_vedge_csc)" ]; then
        docker build -t cnf_vedge_csc . || die "Building docker image failed!"
...
function docker_remove_image () {
    # Remove docker image.
    docker image rm --force cnf_vedge_csc || die "Removing docker image failed!"
    warn "Image removed."
    exit 0
}
