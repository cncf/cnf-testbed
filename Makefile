...
isolcpus_playbook:
	DEPLOY_NAME=$(DEPLOY_NAME) PROJECT_ROOT=$(PROJECT_ROOT) FACILITY=$(FACILITY) VLAN_SEGMENT=$(VLAN_SEGMENT) PLAYBOOK=$(PLAYBOOK) KUBECONFIG=$(KUBECONFIG) ISOLATED_CORES=$(ISOLATED_CORES) tools/kubernetes_provisioning.sh isolcpus

.PHONY: vswitch
vswitch: vswitch_tools

vswitch_tools:
	DEPLOY_NAME=$(DEPLOY_NAME) PROJECT_ROOT=$(PROJECT_ROOT) FACILITY=$(FACILITY) VLAN_SEGMENT=$(VLAN_SEGMENT) PLAYBOOK=$(PLAYBOOK) KUBECONFIG=$(KUBECONFIG) ISOLATED_CORES=$(ISOLATED_CORES) tools/kubernetes_provisioning.sh vswitch
